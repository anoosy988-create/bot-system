...
if (!TOKEN) {
    console.error('❌ TOKEN أو DISCORD_TOKEN غير موجود في Environment Variables.');
    process.exit(1);
}
    ...
    new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName('invites')
                ...
                .addStringOption(o =>
                    o.setName('action')
                        .setDescription('العقوبة أول ما ينشال الاختصار')
                        .addChoices(...PROTECTION_ACTIONS)
                        .setRequired(false)
                )
        )
        .addSubcommand(sub =>
            sub.setName('scams')
                .setDescription('حماية النصب: روم محدد — أي كلام/صورة/رابط فيه = عقوبة فورية')
                .addBooleanOption(o =>
                    o.setName('enabled')
                        .setDescription('تفعيل الحماية')
                        .setRequired(true)
                ...
                .addChannelOption(o =>
                    o.setName('channel')
                        .setDescription('الروم المحمي (يبدل الاختيار القديم)')
                        .addChannelTypes(ChannelType.GuildText)
                        .setRequired(false)
                )
                .addBooleanOption(o =>
                    o.setName('clear')
                        .setDescription('امسح الروم المحمي (تعطيل room بدون لمس القواعد)')
                        .setRequired(false)
                )
                .addBooleanOption(o =>
                    o.setName('talk')
                        .setDescription('عقوبة على أي كلام بالـ روم')
                        .setRequired(false)
                )
                .addBooleanOption(o =>
                    o.setName('image')
                        .setDescription('عقوبة على إرسال صورة')
                        .setRequired(false)
                )
                .addBooleanOption(o =>
                    o.setName('link')
                        .setDescription('عقوبة على إرسال رابط')
                        .setRequired(false)
                )
                .addStringOption(o =>
                    o.setName('action')
...
function dashboardBaseUrl() {
    return String(DASHBOARD_URL || '').replace(/\/+$/, '');
}

function dashboardLinkLine() {
    return DASHBOARD_URL ? `${dashboardBaseUrl()}/#servers` : null;
}
...
async function handleDashboardCommand(interaction) {
    ...
    if (sub === 'activate') {
        ...
        if (!managed.length) {
            return interaction.reply({
                content:
                    '❌ ما لقيت لك أي سيرفر عندك فيه صلاحية **Admin** أو رتبة **ستريتر**.\n' +
                    'تأكد أن البوت موجود بالسيرفر وأن رتبتك فوق رتبة البوت، وبعدها أعد الأمر.',
                ephemeral: true
            });
        }
        ...
        const dmText =
            '✅ **تم تفعيل حسابك في الداشبورد**\n\n' +
            `🔗 رابط دخولك: ${link}\n\n` +
            'بمجرد ما تفتح الرابط اضغط **دخول بحساب Discord** (بدون آيدي وبدون كود)، ' +
            'وتطلع لك السيرفرات اللي تملك فيها صلاحية:\n' +
            managed.map(g => `• ${g.name}`).join('\n') +
            '\n\n> جلسة الدخول تدوم أسبوع، وتقدر تسجل خروج وقت ما تحب.';
        ...
        return interaction.reply({
            content: delivered
                ? `✅ تم تفعيل حسابك — أرسلت لك رابط الدخول بالخاص.\n\n${link}`
                : `✅ تم تفعيل حسابك، بس ما قدرت أرسل لك رسالة خاصة (افتح الرسائل المباشرة من ديسكورد).\n\nخلك هنا ونسخ الرابط:\n${link}`,
            ephemeral: true
        });
    }
    ...
    if (sub === 'list') {
        ...
        if (!records.length) {
            return interaction.reply({ content: '📭 ما فيه أي حساب مفعّل بعد.\nأرسلوا `/dashboard activate` بالسيرفرات.', ephemeral: true });
        }
        ...
        return interaction.reply({
            content: `📋 الحسابات المفعّلة (${records.length}):\n${lines.join('\n')}`,
            ephemeral: true
        });
    }
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            const isProtectionCommand = command === 'protect';
